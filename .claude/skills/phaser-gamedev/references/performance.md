...
## Audio: Tab-Refocus Sound Stacking

When the browser tab is backgrounded, Phaser's game loop pauses but game state may still queue updates. On refocus, all queued combat ticks fire at once, each calling `sound.play()`. Dozens of overlapping sounds create an ear-blasting burst.

### Fix: Throttle SFX by Key

```typescript
private sfxLastPlayed: Map<string, number> = new Map()
private static readonly SFX_THROTTLE_MS = 80

private playSfx(key: string, baseVolume: number): void {
  if (this.isMuted) return
  const vol = baseVolume * getSfxVolume()
  if (vol <= 0) return

  // Throttle: skip if this sound played within the last N ms
  const now = performance.now()
  const last = this.sfxLastPlayed.get(key) ?? 0
  if (now - last < SFX_THROTTLE_MS) return
  this.sfxLastPlayed.set(key, now)

  this.sound.play(key, { volume: vol })
}
```

**80ms throttle** prevents stacking while still allowing rapid combat sounds during normal gameplay (~12 sounds/sec max per key).
