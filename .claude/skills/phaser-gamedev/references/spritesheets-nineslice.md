...
**CRITICAL**: Before slicing, check for **spacing between columns**. Many asset packs (e.g., Tiny Swords) use uniform frame sizes with gaps. If you assume the columns are contiguous (e.g., 128+192+128=448), you'll pull transparent gutters into your source regions, causing visible gaps in the composed button. Always verify: `imageWidth = (frameWidth × cols) + (spacing × (cols - 1))`. For example, 448px wide with 3 columns and 8px spacing = 144px frames, NOT 128+192+128.
